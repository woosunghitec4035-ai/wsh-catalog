(function(){
 ...
 var panels={
  architecture:['SYSTEM ARCHITECTURE','팜시스 시스템 구성','센서 측정부터 현장 자동제어와 원격관리까지의 연결 구조입니다.','<img class="fs-wide-image" src="assets/img/farmsys-system-map.png" alt="팜시스 시스템 구성도"><p class="fs-dialog-note"><b>제어 흐름:</b> 환경센서가 측정정보를 전달하면 팜시스 컨트롤러가 설정값에 따라 현장의 로컬 컨트롤러와 설비를 제어합니다. 서버와 인터넷 연결은 원격조회·데이터 저장·CCTV 확인을 지원합니다.</p>'],
  ...
  types:['CONTROLLER CONFIGURATION','통합형과 분리형','현장 규모와 기존 제어반 구성에 따라 컨트롤러 형식을 선택합니다.','<div class="fs-type-grid"><article><img src="assets/img/farmsys-integrated.png" alt="팜시스 통합형 컨트롤러"><h3>통합형 · Integrater Type</h3><p>현장 기계장치의 수량과 용량에 맞춰 제작하는 주문형 제어반입니다. 임베디드 컴퓨터·릴레이보드·센서 수신장치·마그네틱 콘텍트·수동 스위치·출력단자가 일체로 구성되어 <strong>환경조절 기계장치를 제어반에 직접 연결</strong>합니다.</p></article><article><img src="assets/img/farmsys-component.png" alt="팜시스 분리형 컨트롤러"><h3>분리형 · Component Type</h3><p>FSC-1520의 규격화된 중앙 컨트롤러에서 무전압 릴레이 신호를 내보내고, 연결된 로컬 컨트롤러가 실제 구동회로를 제어합니다. <strong>현재 사용 중인 로컬 제어기를 활용하거나 계통별로 구성</strong>할 때 적합합니다.</p></article></div>'],
  sensors:['ENVIRONMENT SENSORS','환경센서 구성','재배환경과 설치 목적에 맞는 센서 및 노드를 구성합니다.','<div class="fs-sensor-grid"><article><img src="assets/img/farmsys-indoor-sensors.png" alt="실내 공기환경 통합센서"><h3>실내 공기환경 통합센서</h3><p>FSS-701은 온도·습도·CO₂, FSS-702는 온도·습도를 측정합니다. 최대 4대와 유선 예비온도센서 1개를 사용할 수 있습니다.</p></article><article><img src="assets/img/farmsys-weather-sensors.png" alt="기상센서"><h3>외부 기상센서</h3><p>풍향·풍속·일사·강우와 외부 온·습도를 측정합니다. 외부 기상센서를 모두 사용할 때는 기상센서 노드가 필요합니다.</p></article><article><img src="assets/img/farmsys-rain-sensor.png" alt="3면 입체감지 강우센서"><h3>강우센서</h3><p>3면 입체감지 방식으로 강우 유무를 확인합니다. 코드 길이는 7m이며, 강우조건에 따른 환기창 비상제어에 활용합니다.</p></article></div><p class="fs-dialog-note">※ 배지센서는 수경재배용 인공배지의 수분과 온도를 측정합니다. 일반 토양의 수분을 측정하는 센서가 아닙니다.</p>'],
  operation:['OPERATION & REMOTE MANAGEMENT','사용 화면과 원격관리','현장 터치화면을 중심으로 운전하며 스마트폰과 PC에서 상태와 데이터를 확인합니다.','<img class="fs-wide-image" src="assets/img/farmsys-operation.png" alt="팜시스 현장·스마트폰·PC 사용 화면"><p class="fs-dialog-note">원격제어는 농장 환경을 스마트폰으로 계속 조작하는 기능이 아니라, 자동제어 상태를 확인하고 이상 발생 시 조회·응급운전을 돕는 보조 기능입니다.</p>']
 };
